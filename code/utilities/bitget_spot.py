...
class BitgetSpot():
    ...
    def fetch_closed_orders(self, symbol: str) -> List[Dict[str, Any]]:
        try:
            return self.session.fetch_closed_orders(symbol)
        except Exception as e:
            raise Exception(f"Failed to fetch closed orders: {e}")
    ...
    def place_limit_order(self, symbol: str, side: str, amount: float, price: float, reduce: bool = False) -> Dict[str, Any]:
        try:
            ...
            return self.session.create_order(symbol, 'limit', side, amount, price)
